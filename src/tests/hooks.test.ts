...
describe('useGetImage', () => {
  it('should fetch image successfully', async () => {
    ...
    waitFor(()=>{expect(result.current.imageSrc).toBe(getAddress(imageAddress));
    expect(result.current.isLoading).toBe(false);
    expect(result.current.error).toBe(null);},{timeout:1000})
  ...
  it('should handle failed image fetch', async () => {
    ...
    waitFor(()=>{    expect(result.current.imageSrc).toBe(null);
    expect(result.current.isLoading).toBe(false);
    expect(result.current.error).toBe('Failed to fetch image');}, {timeout:1000})
